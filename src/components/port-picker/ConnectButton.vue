...
async function loadSerialBackend() {
    return await import("../../js/serial_backend");
}

async function selectAndConnect(path) {
    PortHandler.portPicker.selectedPort = path;
    const { connectDisconnect } = await loadSerialBackend();
    connectDisconnect();
}
    ...
    setup() {
        ...
        return {
            ...
            onDisconnectClick: async () => {
                const { disconnect } = await loadSerialBackend();
                disconnect();
            },
            onDialogConfirm,
        };
